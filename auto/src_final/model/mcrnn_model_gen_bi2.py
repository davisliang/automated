# This is different from mcrnn_model_gen_bi.py in that only the shared lstm is bidirectional
...
class model(object):
        ...
        is_bidirectional = True  #if the body lstm is bidirectional or not; the context and task lstms are always single dimensional
	fc_activation = "tanh"
	...
	keep_prob_val = 1.0
        ...
        def rnn_layer(self, fw_cell, bw_cell, rnn_inputs, is_bidirectional=False):
            if is_bidirectional:
	...
	def buildModel(self, x, y_context, y_task, is_train, dropout, scope="multiTask"):
	    ...
	    if self.is_bidirectional:
		    ...
		    context_cell_output, last_context_state = self.rnn_layer(fw_cell=context_lstm_cell, bw_cell=None, rnn_inputs=context_input)
		...
		if self.is_bidirectional:
		    ...
		    task_input = body_cell_output
                task_cell_output, last_task_state = self.rnn_layer(fw_cell=task_lstm_cell, bw_cell=None, rnn_inputs=task_input)
