...
def get_text_by_batch(data_path, is_train, is_val, train_file, test_file, val_file, all_classes, start_idx, batch_size):
    if is_train:
        identifiers = train_file
    elif is_val:
        identifiers = val_file
    else:
...
def load_data(data_path):
    ...
    val_file = get_file_identifiers(data_path + "/validation_classes.txt")
    return all_classes, train_file, test_file, val_file
...
def load_batch(n_classes, word2vec_dic, missing_word_dic, encode_dim, max_len, data_path, is_train, is_val, train_file, test_file, val_file,all_classes, start_idx, batch_size, automated_task):
    batch_identifiers, batch_text = get_text_by_batch(data_path, is_train,is_val,  train_file, test_file, val_file, all_classes, start_idx, batch_size)
...
if __name__ == "__main__":
    ...
    n_classes, n_data, n_data_per_class, n_train_data, n_test_data, max_length = data_stats['n_classes'], data_stats['n_data'], data_stats['n_data_per_class'],data_stats['n_train_data'], data_stats['n_test_data'], data_stats['max_length']
    print n_classes, n_data, n_data_per_class
    ...
    for epoch in range(3):
        ...
        for minibatch in range(3):
            encoded_batch, batch_classes, batch_context_encoded, batch_context, batch_identifier, batch_text, batch_length = load_batch(n_classes, word2vec_dic, {}, 300, max_length, data_path+"/Train/", 1, train_file, test_file, all_classes, start_idx, 1, automated_task="word generation")
            start_idx += 1
            print batch_text
            print batch_classes
            print batch_context
            print encoded_batch.shape
            print batch_context_encoded.shape
