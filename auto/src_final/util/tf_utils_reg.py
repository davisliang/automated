...
def fcLayer(x, in_shape, out_shape, activation, dropout, is_train, reg_const, scope="fc"):
    
    x = tf.reshape(x, [-1, in_shape])
 
    with tf.variable_scope(scope):
        w = tf.get_variable(name="w", shape = [in_shape, out_shape], dtype=tf.float32, initializer=tf.truncated_normal_initializer(stddev=1e-2), regularizer=tf.contrib.layers.l2_regularizer(reg_const))
...
def predictionLayer(x, y, in_shape, out_shape, activation, reg_const, scope="prediction"):
    
    x = tf.reshape(x, [-1, in_shape])

    with tf.variable_scope(scope):
        w = tf.get_variable(name=scope+"w", shape = [in_shape, out_shape], dtype=tf.float32, initializer=tf.truncated_normal_initializer(stddev=1e-2), regularizer=tf.contrib.layers.l2_regularizer(reg_const))
