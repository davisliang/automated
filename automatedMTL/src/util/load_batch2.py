...
def load_batch(n_classes, word2vec_dic, missing_word_dic, encode_dim, max_len, data_path, is_train, train_file, test_file, all_classes, start_idx, batch_size):
    ...
    batch_classes = np.zeros((batch_size, 2))
    batch_missing_word_encoded = np.zeros((batch_size, encode_dim))
    batch_missing_word = []
    batch_length = []
    for i in range(batch_size):
        ...
        batch_missing_word_encoded[i,:] = word2vec_dic[missing_word_dic[batch_identifiers[i]]]
        batch_missing_word.append(missing_word_dic[batch_identifiers[i]])
        batch_length.append(text_length)
    return encoded_batch, batch_classes, batch_missing_word_encoded, batch_missing_word, batch_identifiers, batch_text, batch_length
...
if __name__ == "__main__":
    ...
    for epoch in range(3):
        ...
        for minibatch in range(73):
            encoded_batch, batch_classes, batch_missing_word_encoded, batch_missing_word, batch_identifier, batch_text, batch_length = load_batch(n_classes, word2vec_dic, missing_word_dic, 300, max_length, data_path+"/Train/", 1, train_file, test_file, all_classes, start_idx, 128)
            start_idx += 128
            #print batch_text
            #print batch_missing_word
            #print batch_length
