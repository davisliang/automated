...
data_path = expanduser("~/tweetnet/data/dump.txt")
...
def get_hashmap_prep(vocab_path):
    ...
    with open(expanduser("~/tweetnet/data/hashmap_prep.pkl"), "wb") as ff:
    ...
    for hashtag in hash_dict.keys():
        ...
        for h in dic.keys():
            if h not in all_vocab:
                dic.pop(h)
    with open(expanduser("~/tweetnet/data/hashmap1.pkl"), "wb") as f:
...
def word2vec_dict(file_path):
    ...
    with open(file_path, "rb") as lines:
        for line in lines:
            ...
            count += 1
    with open(expanduser("~/tweetnet/data/word2vec_dict.pkl"), "wb") as f:
...
def generate_vector(hash_map, word2vec):
    ...
    for k in hashmap.keys():
        if hashmap[k] == {}:
            ...
            if hashmap[k] == {}:
                hashmap.pop(k)
    with open(expanduser("~/tweetnet/data/hashmapfinal.pkl"), "wb") as ff:
    ...
    with open(expanduser("~/tweetnet/data/word2vec_additional.pkl"), "wb") as fff:
...
def tweet2vector(dict1_path, dict2_path):
    ...
    with open(expanduser("~/tweetnet/data/hashtag_embedding.pkl"),"wb") as fff:
...
def remove_invalid_tweets(embed):
    ...
    with open(expanduser("~/tweetnet/data/new_embeddings.pkl"), "wb") as fff:
        pickle.dump(embeddings, fff, pickle.HIGHEST_PROTOCOL)

if __name__ == "__main__":
    get_unique_hashtags()
    get_hashmap_prep(expanduser("~/tweetnet/data/vocabs.txt"))
    generate_vector(expanduser("~/tweetnet/data/hashmap1.pkl"), expanduser("~/tweetnet/data/word2vec_dict.pkl"))
    tweet2vector(expanduser("~/tweetnet/data/word2vec_dict.pkl"), expanduser("~/tweetnet/data/word2vec_additional.pkl"))
