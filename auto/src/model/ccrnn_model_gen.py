...
class model(object):
	...
	def buildModel(self, x, y_context, y_task, is_train, dropout, scope="multiTask"):
	    ...
	    # body_lstm_cell, _ = createGRUCell(self.batch_size, self.body_lstm_size)
	    ...
	    # context_lstm_cell, _ = createGRUCell(self.batch_size, self.context_lstm_size)
	    ...
	    # task_lstm_cell, _ = createGRUCell(self.batch_size, self.task_lstm_size)
            ...
            if self.is_multi_task:
        	    ...
        	    context_cell_output, last_context_state = tf.nn.dynamic_rnn(cell = context_lstm_cell, dtype=tf.float32, sequence_length=self.length(x), inputs=x)
		    task_input = tf.concat([body_cell_output, context_cell_output], 2)
			...
			with tf.variable_scope("context_fc"):
			        ...
			        fc_out = fcLayer(x=context_output_list[step], in_shape=self.context_lstm_size, out_shape=self.context_branch_fc, activation=self.fc_activation, dropout=self.dropout, is_train=is_train, scope="fc1")
        	...
        	task_cell_output, last_task_state = tf.nn.dynamic_rnn(cell = task_lstm_cell, dtype=tf.float32, sequence_length=self.length(task_input), inputs=task_input)
